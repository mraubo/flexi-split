...
- Endpoint POST szkic:  
  ...
  export const POST: APIRoute = async (ctx) => {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "validation_error", details: { body: "invalid json" } }), { status: 400 });
    }

    const parsed = CreateSettlementSchema.safeParse(payload);
    if (!parsed.success) {
      return new Response(JSON.stringify({ error: "validation_error", details: parsed.error.flatten().fieldErrors }), { status: 400 });
    }
    ...
    try {
      ...
      if (e?.code === "MAX_OPEN_SETTLEMENTS") {
        return new Response(JSON.stringify({ error: "business_rule_violation", code: "MAX_OPEN_SETTLEMENTS" }), { status: 422 });
      }
      if (e?.status === 401) {
        return new Response(JSON.stringify({ error: "unauthorized" }), { status: 401 });
      }
  ...
  export class SettlementService {
    ...
    async create(input: { title: string }, userId: string) {
      const { data, error } = await this.supabase
        ...
        .select("id,title,status,currency,participants_count,expenses_count,created_at,updated_at,closed_at,last_edited_by")
        .single();
